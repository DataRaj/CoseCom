...
export default async function SuccessPage({
  ...
  searchParams: Promise<{ name?: string; email?: string; phoneNumber?: string; short_url?: string }>;
}) {
    ...
    <div className="flex flex-col items-center justify-center gap-8 px-2 py-16 md:py-24">
      ...
      <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-secondary p-4">
        <p className="text-center text-lg">
          ...
          <span className="underline">{params.email ?? "N/A"}</span> and{" "}
          <span className="underline">{params.phoneNumber ?? "N/A"}</span>
        </p>
        <div className="mt-4 flex flex-row gap-2">
          <a href={params.short_url ?? "#"} target="_blank" rel="noopener noreferrer">
